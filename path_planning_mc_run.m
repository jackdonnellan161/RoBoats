...
%% MC Setup
% E3 = E3_test(U_field,V_field,lambda_field/(length(xs)-1),b_field/(length(ys)-1));
...
% for ii = 1:length(ys)
%     U_field(ii,:) = (2/max(ys))*(ys' - max(ys)/2);
% end
% surf(xs,ys,U_field)
% shading interp
% for ii = 1:length(xs)
%     V_field(:,ii) = (2/max(xs))*(max(xs)/2 - xs);
% end
% 
% y1 = max(ys) + max(ys)/max(xs).*(xs-max(xs));
% y2 = max(ys) - max(ys)/max(xs).*xs;
% 
% for jj = 1:length(ys)
%     for ii = 1:length(xs)
%         if (ys(jj) > y1(ii) && ys(jj) > y2(ii)) || (ys(jj) < y1(ii) && ys(jj) < y2(ii))
%             V_field(ii,jj) = 0;
%         else
%             U_field(ii,jj) = 0;
%         end
...
% V_field = temp;
% figure
% quiver(xs/lambda_field,ys/b_field,U_field,V_field)
x_dist = lambda_field/(length(xs));
y_dist = b_field/(length(ys));
% 
% U_field_load = x_dist*ones(size(U_field_load));
% V_field_load = y_dist*ones(size(V_field_load));
E3 = E3_test_2(flip(U_field_load),flip(V_field_load),x_dist,y_dist);
% E3 = E3_test_2(flip(U_field),flip(V_field),x_dist,y_dist);
% monte_carlo_get_points()

%Specify in nx ny
start_points = [round(16*nx/100) round(64*ny/100)];
end_points = [round(16*nx/100) round(37*ny/100)];


xy = combvec([1:1:length(xs)],[1:1:length(ys)])';
for ii = 1:length(start_points(:,1))
    s = start_points(ii,:); 
    e = end_points(ii,:);
    startpoint = sub2ind([length(U_field_load(1,:)) length(U_field_load(:,1))],round(s(1)),round(s(2)))
    endpoint = sub2ind([length(U_field_load(1,:)) length(U_field_load(:,1))],round(e(1)),round(e(2)))
    
    tic
    [cost,path] = dijkstra(xy,E3,startpoint,endpoint);
%     [cost,path] = dijkstra(xy,E3,endpoint,startpoint);
    toc
    key_points = xy(path,:);
    waypoints_load = [xs(key_points(:,1))', ys(key_points(:,2))'];
%     figure
%     scatter(global_points{ii}(:,1),global_points{ii}(:,2)
    allow_disable = 0;
...
%     sim('Boat_Simulation_initial_QG_flow_trials_actually_working')
end

figure
% contour(xs,ys,Psi_in,10)
quiver([1:1:nx],[1:1:ny],U_field_load,V_field_load);
hold on
scatter(key_points(:,1),key_points(:,2))
scatter(start_points(:,1),start_points(:,2))
scatter(end_points(:,1),end_points(:,2))
contour([1:1:nx],[1:1:ny],Psi_in,10)
...
%     s = start_points(ii,:); 
%     e = end_points(ii,:);
%     startpoint = sub2ind([length(U_field(1,:)) length(U_field(2,:))],round(s(1)),round(s(2)));
%     endpoint = sub2ind([length(U_field(1,:)) length(U_field(2,:))],round(e(1)),round(e(2)));
%     tic
%     [cost,path] = dijkstra(xy,E3,startpoint,endpoint);
%     toc
%     global_points{ii} = xy(path,:);
%     
% %     figure
% %     scatter(global_points{ii}(:,1),global_points{ii}(:,2)
%     
